...
namespace kop_launcher
{
    class ConfigFileReaderWriter
    {
        public Dictionary<string, string> ReadConfigFile(string fileName)
        {
            ...
            try
            {
                string path = Path.Combine(Globals.rootDirectory, "system", "CandyFX", fileName);
                string line = "";
                ...
                using (StreamReader Reader = new StreamReader(path))
                {
                    while ((line = Reader.ReadLine()) != null)
                    {
                        ...
                        {
                            string value = line.Trim().Split(' ').Last();
                            string key = line.Replace(value, "").Replace("#define ", "").Trim();
        ...
        public Dictionary<string, string> ReadCoreIniSettings(string path)
        {
            ...
            try
            {
                if (File.Exists(path))
                {
                    IniFile iniReader = new IniFile(path);
        ...
        {
            StringBuilder sb = new StringBuilder();
            string fileName = Path.Combine(Globals.rootDirectory, "user", "system.ini");
        ...
        public bool OverrideCamera(short num)
        {
            ...
            try
            {
                string regPath = Path.Combine(Globals.rootDirectory, "scripts", "lua", "table", "override", "camera");
                string tempPath = "";
                string destination = Path.Combine(Globals.rootDirectory, "scripts", "lua", "CameraConf1024.clu"); 
                switch (num)
                {
                    case 1:
        ...
        public bool OverrideCamera(bool regular)
        {
            try
            {
                string regPath = Path.Combine(Globals.rootDirectory, "scripts", "lua", "table", "override", "animation");
                string tempPath = "";
                string destination = Path.Combine(Globals.rootDirectory, "scripts", "txt", "CharacterAction.tx");
