...
namespace kop_launcher
{
    ...
    {
        private static string[] IllegalSoftware = new string[]
        {
            /* Packet Editors */
            "Tsearch",
            "ArtMoney",
        ...
        public static string IsRunningIllegalSoftware()
        {
            ...
            {
                if (Exclude.Any(proc.ProcessName.ToLowerInvariant().Contains))
                {
                    continue;
                }

                if (IllegalSoftware.Any(s => proc.ProcessName.ToLowerInvariant().Contains(s.ToLowerInvariant())))
                {
        ...
        public static bool IsSystemHashChanged()
        {
            ...
            try
            {
                ...
                {
                    for (int i = 0; i < Globals.GenuineResourceHashes.Count; i++)
                    {
                        if (Globals.GenuineResourceHashes.ElementAt(i) != Globals.CurrentResourceHashes.ElementAt(i))
                        {
                            return true;
                        }
                    }
        ...
        {
            System.Collections.Generic.List<int> holder = new System.Collections.Generic.List<int>();
            try
            {
                if (Globals.GenuineResourceHashes.Count > 0 && Globals.CurrentResourceHashes.Count > 0)
                {
        ...
        {
            string dir = Path.Combine(Globals.rootDirectory, "user");
